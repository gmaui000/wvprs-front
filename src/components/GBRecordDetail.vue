...
<script>
  ...
  import FlvPlayer from './common/FlvPlayer.vue'
  import moment  from 'moment'
  import recordDownload from './dialog/recordDownload.vue'
	export default {
		name: 'app',
		components: {
			uiHeader, FlvPlayer,recordDownload
		},
		data() {
			return {
        ...
        playerHeight: window.innerHeight - 240,
      ...
      gbScale(command){
        ...
        if (this.streamId) {
          this.$axios({
            ...
            this.$refs.recordVideoPlayer.setPlaybackRate(command);
          });
        }
      },
      ...
      getUrlByStreamInfo(){
        if (location.protocol === "https:") {
          this.videoUrl = this.streamInfo["https_flv"]
        }else {
      ...
      updateHeight() {
        const newHeight = window.innerHeight - 240;
        this.playerHeight = newHeight;
        this.playerBoxStyle.height = newHeight + 'px';
        this.recordListStyle.height = newHeight + 'px';
      },
		}
	};
</script>
